...
In the DNA sequence stored in the string dna\_sequence, we want to graph the frequency of each nucleotide.
Sort and fill in the blanks in the following code to get the frequency plot. Notice that we are using the function
nucleotide\_counts, which we constructed in the previous episode  
...
algorithm (spring\_layout). This assigns positions to nodes
in such a way that minimizes the forces between them,
resulting in a visually appealing layout.

When you call nx.spring\_layout(G) without specifying any
...
Try removing or changing the seed\_value; what do you observe?
...
Now, we will use node\_trace to define some characteristics of the nodes in the graph.
...
These traces are stored in a list (edge\_traces) to be later included in the plot.
...
These traces are stored in a list (triangle\_traces) to be later included in the plot.
...
its width and height based on the plot\_size and dpi variables.
...
3) what is the save\_fiename doing?
...
2) Object simplex\_tree contains information about nodes, edges, and triangles
